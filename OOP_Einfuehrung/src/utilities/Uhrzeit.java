...
class Uhrzeit implements Comparable
{
    ...
    {
        if (min < 0)
            throw new IllegalArgumentException("Minuten m�ssen >= 0 sein.");
        else
            while (min >= 60)
                min = min - 60;
        this.min = min;
        if (std < 0)
            throw new IllegalArgumentException("Stunden m�ssen >= 0 sein.");
        else
            while (std >= 24)
                std = std - 24;
        this.std = std;
    ...
    void setzeUhrzeit(int std, int min, int sek)
    {
        setzeStdMin(std, min);
        if (sek < 0 | sek > 59)
            throw new IllegalArgumentException("Sekunden m�ssen zwischen 0 und 59 liegen.");
        this.sek = sek;
        zone = 1;
    }
    ...
    public boolean equals(Object einObject)
    {
        ...
        return this.std - this.zone == eineZeit.std - eineZeit.zone & this.min == eineZeit.min;
    }

    public int compareTo(Object einObject)
    {
        Uhrzeit z = (Uhrzeit) einObject;
        return (((this.std - this.zone) * 60) + this.min) * 60 + this.sek - ((((z.std - z.zone) * 60) + z.min) * 60 + z.sek);
    }
